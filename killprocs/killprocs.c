...
check_pid (char *str, int base)
{
  ...
  if (*end != '\0' || end == str || num <= 0) //PONER ESTE <= EN LINUX
    {
...
wait_children(int *arr, int num_pids, ProcessTuple *tupla_kv){
  ...
  for (i = 0; i < num_pids; i++) {
    ...
    if ((WIFEXITED(status) && WEXITSTATUS(status) == 0) && killed_victim != NULL) {
...
void
kill_procs(int* arr, int num_pids, ProcessTuple *tupla_kv) {
  int i;
  pid_t pid;
  for (i = 0; i < num_pids; i++) {
    pid = fork();
...
main(int argc, char *argv[]) {
    ...
    argv++;
    int* array_pids;
    ProcessTuple *tupla = malloc(argc * sizeof(ProcessTuple)); //
    if (argc == 0) {
        usage();
    }
    //array de PIDS validados.
    array_pids = argv_to_pids(argv, argc);
    kill_procs(array_pids, argc, tupla);
