...
char *
copiar_path (char *path)	// otra opcion es usar strdup(), pero prefiero hacerlo manual para usar malloc()
{
  char *copia_path = (char *) malloc (strlen (path + 1) * sizeof (char));
  for (long i = 0; i < strlen (path); i++)
    {
      copia_path[i] = path[i];
    }
  return copia_path;		// manipular este array
}
...
imprimir_largo (char *copia_path, char *separador)
{
  ...
  for (token = strtok_r (NULL, separador, &end); token;
